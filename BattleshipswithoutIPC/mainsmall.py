...
def switchplayer():                                                                                                     #the switchplayer fuction starts here
    ...
    while playerswitch:                                                                                                 #as long playerswitch is true enter while loop
        for event in pygame.event.get():                                                                                #in case an user event is accuring capture that event
        ...
        if gamesmall.player1turn:                                                                                       #when player 1 is the active player
            ...
            textboxplayer1turn = headline.render(textplayer1turn, False, Green, Black)                                  #the str stored in var textplayer1turn is renderd
            Window.blit(textboxplayer1turn, (Width // 2 - 150, Height // 2 - 50))                                       #the location is set and blitted to the screen
            textboxmissed = headline.render(textmissed, False, Red)                                                     #the str stored in var textboxmissed is renderd
            Window.blit(textboxmissed, (Width // 2 - 80, Height // 2 - 100))                                            #the location is set and blitted to the screen
            ...
            Window.blit(textboxcontinue, (Width // 2 - 150, Height // 2 + 50))                                          #the location is set and blitted to the screen
            pygame.display.update()                                                                                     #display is updated by pygame


        if not gamesmall.player1turn:                                                                                   #when player 2 is the active player
            Window.fill(Black)                                                                                          #the screen will be covered in black
            textmissed = "You missed!"                                                                                  #text is stored in a variable
            ...
            Window.blit(textboxplayer2turn, (Width // 2 - 150, Height // 2 - 50))                                       #the location is set and blitted to the screen
...
gamesmall = Gamesmall()                                                                                                 #the actual game starts here
...
while animating:                                                                                                        #the pygame loop starts
    ...
    for event in pygame.event.get():                                                                                    #in case an user event is occuring capture that event
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:                                                                        #if the event is a click on the mouse
            ...
            if not gamesmall.over and gamesmall.player1turn and x < 6 * Square and y < 6 * Square:                      #as long the game is not over and player one is the active player and the click was within the grid in the top left corner...
                row = y // Square                                                                                       #...divide the row position by the var square so the position matches the grid in the range 0-35...
                col = x // Square                                                                                       #...and divide the column position by the var square so the position matches the grid in the range 0-35...
                index = row * 6 + col                                                                                   #...this is now stored as the actual index which represents the grid, we are working with in this programm
                gamesmall.playerturnsmall(index)                                                                        #the position is now stored to the players index
                if not gamesmall.player1turn:
                    switchplayer()                                                                                      #the switchplayer function is called
            elif not gamesmall.over and not gamesmall.player1turn and x > Width - 6 * Square and y > 6 * Square + Square: #as long the game is not over and player two is the active player and the click was within the grid in the bottom right corner...
                row = (y - Square * 6 - Square) // Square                                                               #...divide the row position(in the bottom right grid) by the var square so the position matches the grid in the range 0-35...
                col = (x - Square * 6 - Square) // Square                                                               #...and divide the column position(in the bottom right grid) by the var square so the position matches the grid in the range 0-35...
                index = row * 6 + col                                                                                   #...this is now stored as the actual index which represents the grid, we are working with in this programm
                gamesmall.playerturnsmall(index)                                                                        #the position is now stored to the players index
                if gamesmall.player1turn:
                    switchplayer()                                                                                      #the switchplayer function is called
        ...
        if event.type == pygame.KEYDOWN:                                                                                #if the event is a pressed key...
            ...
            if event.key == pygame.K_RETURN:                                                                            #if the "Enter" key is pressed...
                gamesmall = Gamesmall()                                                                                 #...restart game
        ...
        if animating:                                                                                                   #in case an event accured prior that set animating to false, the loop stops here and the pygame window closes
            ...
            gridcreation(gamesmall.player1)                                                                             #the grid/Battlefield for player one is drawn with the indexes of the ships of player two (which are invisible at this point of time)
            gridcreation(gamesmall.player2, left=(Width - Square) // 2 + Square,                                        #the grid/Battlefield for player two is drawn with the indexes of the ships of player one (which are invisible at this point of time)
                         top=(Height - Square) // 2 + Square)

            if gamesmall.player1turn:                                                                                   #in case player one is the active player the following happens

                drawshipssmall(gamesmall.player1, left=(Width - Square) // 2 + Square,                                  #the ships of player one are drawn on the grid in the bottom right corner of the window
                               top=(Height - Square) // 2 + Square)
                ...
                Window.blit(textbox, (Width // 2 - 0, Height // 2 - 100))                                               #the location is set and blitted to the screen
                ...
                Window.blit(textbox, (Width // 2 - 150, Height // 2 + 150))                                             #the location is set and blitted to the screen


            elif not gamesmall.player1turn:                                                                             #in case player two is the active player the following happens

                drawshipssmall(gamesmall.player2)                                                                       #the ships of player one are drawn on the grid in the top left corner of the window
                ...
                textyourships = "<- Your Ships"                                                                         #text is stored in var
                textbox = Playerturntext.render(textyourships, False, Green, Black)                                     #the str stored in var textyourships is rendered
                Window.blit(textbox, (Width // 2 - 0, Height // 2 - 150))                                               #the location is set and blitted to the screen


            if gamesmall.over:                                                                                          #in the case game.over is set true the following is happening

                drawshipssmall(gamesmall.player1, left=(Width - Square) // 2 + Square,                                  #all ships are drawn on both grids when game is over...
                               top=(Height - Square) // 2 + Square)                                                     #...first the ships of player one in the bottom right grid...
                drawshipssmall(gamesmall.player2)                                                                       #...and now the ships of player two in the top left grid

                finalresult = "Player " + str(gamesmall.result) + " wins!"                                              #the str with the result is stored in the var "finalresult" including the int from game.result as a string
                textbox = headline.render(finalresult, False, Red, Black)                                               #the str stored in var finalresult is renderd and displayed
                Window.blit(textbox, (Width // 2 - 80, Height // 2 - 0))                                                #the location is set
                ...
                Window.blit(textboxstartagain, (Width // 2 - 100, Height // 2 + 50))                                    #the location is set and blitted to the screen
